...
class RiskEngine:
    def __init__(self, policy: RiskPolicy | None = None) -> None:
        ...
        threshold = profile_thresholds.get(policy_service.profile_name(), 0.6)
        self.policy = policy or RiskPolicy(approval_threshold=threshold)
        self._profile = policy_service.profile_name()
    ...
    def _policy_forces_approval(self, action: ActionEnvelope) -> bool:
        if action.action == ActionType.file_ops:
            op = str(action.parameters.get('op', '')).lower()
        ...
        if action.action == ActionType.open_app:
            ...
            if app_policy.level == 'blocked':
                return True
            if app_policy.level == 'risky' and self._profile != 'aggressive':
                return True
