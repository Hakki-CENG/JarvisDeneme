...
class AgentMesh:
    AGENTS = ('Planner', 'Researcher', 'Tool-Selector', 'Risk-Critic', 'Critic', 'Coder', 'Verifier')
    ...
    def deliberate(self, task: TaskSpec) -> DeliberationOutput:
        ...
        memory_hits = memory_service.search(objective, limit=6)
        memory_context = '\n'.join(
            [
                f'- key={item.key} tags={",".join(item.tags)} score={item.score:.2f} summary={item.content[:180]}'
        ...
        for round_no in range(settings.max_reasoning_rounds):
            for agent in self.AGENTS:
                prompt = (
                    ...
                    f'RelevantMemory={memory_context or "- none"}'
                )
    ...
    def _extract_json_object(text: str) -> dict | None:
        ...
        for block in fenced_blocks:
            parsed = AgentMesh._first_json_object(block)
            if parsed is not None:
                return parsed
        return AgentMesh._first_json_object(text)
