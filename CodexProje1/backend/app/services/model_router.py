...
class ModelRouter:
    ...
    def _call_gemini(self, prompt: str, round_name: str) -> str:
        api_key = settings.gemini_api_key or secret_vault.get_secret('GEMINI_API_KEY')
        if not api_key:
    ...
    def _call_openrouter(self, prompt: str, round_name: str) -> str:
        api_key = settings.openrouter_api_key or secret_vault.get_secret('OPENROUTER_API_KEY')
        if not api_key:
            raise ProviderRequestError('missing API key')
        ...
        try:
            response = httpx.post('https://openrouter.ai/api/v1/chat/completions', json=payload, headers=headers, timeout=45)
        except Exception as exc:
            raise ProviderRequestError(str(exc)) from exc
